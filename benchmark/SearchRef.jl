...
function runSearchRef(dataFile::AbstractString, numInput::Int64, rootPath::AbstractString)

  println("\n$dataFile begins @ $(Dates.hour(now())):$(Dates.minute(now())):$(Dates.second(now()))")
  resultFilePath = "$rootPath/SearchRef/SearchRef$dataFile.csv"
  dataPath = "$rootPath/RichmondDataSet/$dataFile.csv" # input-output data source
  ...
  aSet = Array{Int64}(0)
  println("initialization time = $runTime")
  ...
  println("$dataFile done! Taking $runTime secs")
  # recording time
  resultFile = open(resultFilePath, "a")
...
function SearchRef(k::Int64, dataMatrix::Matrix{Float64}, numInput::Int64, env::Gurobi.Env; incrementSize = 150, M = 10.0, tol = 10.0^-6, lpUB = 0, initialSize = 0, maxIteration = 300, initialSampling = "random", extremeValueSet=Array{Int64}(0))
    ...
    # tol (default: 10.0^-6): optimal tolerance for the stopping criterion of searchRef()
        # Note that M>1 yields loss in numerical accuracy of the efficiency determination. For example, M=10 and tol=10^-6 give accuracy of 10^-5
